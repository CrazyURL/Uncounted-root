# A2 File Scan Quality Review & AudioDecoderPlugin
...
- **Audio Decoder Bridge** (audioDecoderBridge.ts) -- 2 connections
  - -> related_to -> [[cwe-209]]
  - <- references <- [[a2-file-scan-quality-review]]
- **Audio File Loader** (audioFileLoader.ts) -- 2 connections
  - -> related_to -> [[cwe-209]]
  - <- references <- [[a2-file-scan-quality-review]]
...
- **Wav Encoder** (wavEncoder.ts) -- 2 connections
  - -> related_to -> [[unresolvedrefcwe200]]
  - <- references <- [[a2-file-scan-quality-review]]
- **WavParser** (WavParser.java) -- 2 connections
  - -> related_to -> [[cwe-190]]
  - <- references <- [[a2-file-scan-quality-review]]
- **__unresolved__::ref::cwe_200** () -- 1 connections
...
- Wav Encoder -> related_to -> __unresolved__::ref::cwe_200 [EXTRACTED]
- WavParser -> related_to -> CWE-190 [EXTRACTED]

## Cross-Community Connections

## Context
이 커뮤니티는 A2 File Scan Quality Review, AudioDecoderPlugin, Audio Dedupe를 중심으로 related_to 관계로 연결되어 있다. 주요 소스 파일은 A2-file-scan-quality-review.md, AudioDecoderPlugin.java, AudioProcessor.java, OnnxSttRunner.java, WavParser.java이다.
