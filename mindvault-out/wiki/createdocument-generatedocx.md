...
- **__unresolved__::ref::add_page_break** () -- 2 connections
  - <- calls <- [[createdocument]]
  - <- calls <- [[createdocument]]
- **__unresolved__::ref::datetime** () -- 2 connections
  - <- imports <- [[sttcolab]]
  - <- imports <- [[instinct-cli]]
...
- stt_colab -> imports -> __unresolved__::ref::json (-> [[unresolvedrefgetitem-unresolvedrefsetitem]])
...
- make_overview_table -> calls -> __unresolved__::ref::endswith (-> [[unresolvedrefresolve-runpiiforall]])
...
## Context
이 커뮤니티는 create_document, generate_docx, generate_gstar_docx를 중심으로 calls 관계로 연결되어 있다. 주요 소스 파일은 generate_docx.py, generate_gstar_docx.py, stt_colab.py이다.
