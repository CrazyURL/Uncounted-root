# 전처리 파이프라인 재활성화 (Round 1~4) & Round 4 — dedup 추가 (⏳ 대기)
Cohesion: 0.08 | Nodes: 36

## Key Nodes
- **전처리 파이프라인 재활성화 (Round 1~4)** (uncounted-docs/voice-api/전처리_파이프라인_재활성화.md) -- 9 connections
  - -> contains -> [[why]]
  - -> contains -> [[how]]
  - -> contains -> [[phase-1]]
  - -> contains -> [[phase-2]]
...
- **Round 4 — dedup 추가 (⏳ 대기)** (uncounted-docs/voice-api/전처리_파이프라인_재활성화.md) -- 9 connections
...
- **Round 2 — gain + silence compression** (uncounted-docs/voice-api/전처리_파이프라인_재활성화.md) -- 8 connections
  ...
  - <- contains <- [[round-14]]
- **bash** (uncounted-docs/voice-api/전처리_파이프라인_재활성화.md) -- 7 connections
...
- **Phase 1 — 토글 인프라** (uncounted-docs/voice-api/전처리_파이프라인_재활성화.md) -- 4 connections
  ...
  - <- contains <- [[round-14]]
- **🐛 추가 발견 — utterance_segmenter Case C 과병합 버그** (uncounted-docs/voice-api/전처리_파이프라인_재활성화.md) -- 3 connections
  - -> references -> [[unresolvedrefunresolvedrefunresolvedref]]
  - -> references -> [[unresolvedrefunresolvedref]]
  - <- contains <- [[round-2-gain-silence-compression]]
...
## Context
이 커뮤니티는 전처리 파이프라인 재활성화 (Round 1~4), Round 4 — dedup 추가 (⏳ 대기), Round 2 — gain + silence compression를 중심으로 contains 관계로 연결되어 있다. 주요 소스 파일은 전처리_파이프라인_재활성화.md이다.
